...
4. Style the component with Tailwind utility classes: a centered card with a clear heading ("Table of 22"), comfortable spacing between rows, alternating row shading for readability, and a bold result number. Expected outcome: the table looks tidy and is easy to read on screen.

5. Create a page at `/app/src/pages/TablePage.tsx` that places the table component in the center of the screen with a pleasant background and page title. Use the table component to render the table of 22 from 1 through 10. Expected outcome: opening the app shows the multiplication table of 22 nicely centered.
...
7. (Optional polish) Add a small control to switch the range between "up to 10" and "up to 20". Expected outcome: the visitor can choose how many rows of the table to view.
